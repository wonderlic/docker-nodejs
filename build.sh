...
if [ ! -f node-v4.4.3-linux-x64.tar.gz ]; then
    wget http://nodejs.org/dist/v4.4.3/node-v4.4.3-linux-x64.tar.gz
fi
tar -zxf node-v4.4.3-linux-x64.tar.gz node-v4.4.3-linux-x64/bin/node
mv node-v4.4.3-linux-x64/bin/node .
rm -rf node-v4.4.3-linux-x64

docker build -t wonderlic/nodejs:4.4.3 .

cd test
docker build -t wonderlic/hello_world:4.4.3 .
docker run wonderlic/hello_world:4.4.3
cd ..
